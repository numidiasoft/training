###########################
# VIRTUAL MACHINE SCALE SET
###########################

locals {
  instance_count = 1
}

data "azurerm_image" "search" {
  name                = "blue-green-1561805857"
  resource_group_name = "${azurerm_resource_group.rg.name}"
}

resource "azurerm_virtual_machine_scale_set" "blue" {
  count               = var.enable_blue ? 1 : 0
  name                = "blue"
  location            = "${azurerm_resource_group.rg.location}"
  resource_group_name = "${azurerm_resource_group.rg.name}"

  # Manual upgrade
  upgrade_policy_mode = "Manual"

  # required when using rolling upgrade policy
  // health_probe_id = "${azurerm_lb_probe.test.id}"
  zones = [1, 2, 3]

  sku {
    name     = "Standard_B1s"
    tier     = "Standard"
    capacity = "${local.instance_count}"
  }

  storage_profile_image_reference {
    id = "${data.azurerm_image.search.id}"
  }

  storage_profile_os_disk {
    name              = ""
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }

  storage_profile_data_disk {
    lun           = 0
    caching       = "ReadWrite"
    create_option = "Empty"
    disk_size_gb  = 10
  }

  os_profile {
    computer_name_prefix = "blue"
    admin_username       = "myadmin"

    custom_data = <<EOF
#! /bin/bash
node /home/packer/app/server.js
EOF
  }

  os_profile_linux_config {
    disable_password_authentication = true

    ssh_keys {
      path = "/home/myadmin/.ssh/authorized_keys"
      key_data = "ssh-rsa  tarik@bearer.sh"
    }
  }

  network_profile {
    name = "blue"
    primary = true

    ip_configuration {
      name = "BlueIPConfiguration"
      primary = true
      subnet_id = "${azurerm_subnet.subnet1.id}"
      load_balancer_backend_address_pool_ids = ["${azurerm_lb_backend_address_pool.blue.0.id}"]

      public_ip_address_configuration {
        name = "blue-green"
        domain_name_label = "blue-green"
        idle_timeout = 30
      }
    }
  }

  tags = {
    environment = "Blue Green"
  }
}

resource "azurerm_monitor_autoscale_setting" "blue" {
  count = var.enable_blue ? 1 : 0
  name = "autoscale-cpu"
  target_resource_id = "${azurerm_virtual_machine_scale_set.blue.0.id}"
  location = "${azurerm_resource_group.rg.location}"
  resource_group_name = "${azurerm_resource_group.rg.name}"

  profile {
    name = "autoscale-cpu"

    capacity {
      default = "${local.instance_count}"
      minimum = 1
      maximum = 3
    }

    rule {
      metric_trigger {
        metric_name = "Percentage CPU"
        metric_resource_id = "${azurerm_virtual_machine_scale_set.blue.0.id}"
        time_grain = "PT1M"
        statistic = "Average"
        time_window = "PT5M"
        time_aggregation = "Average"
        operator = "GreaterThan"
        threshold = 75
      }

      scale_action {
        direction = "Increase"
        type = "ChangeCount"
        value = "1"
        cooldown = "PT1M"
      }
    }

    rule {
      metric_trigger {
        metric_name = "Percentage CPU"
        metric_resource_id = "${azurerm_virtual_machine_scale_set.blue.0.id}"
        time_grain = "PT1M"
        statistic = "Average"
        time_window = "PT5M"
        time_aggregation = "Average"
        operator = "LessThan"
        threshold = 15
      }

      scale_action {
        direction = "Decrease"
        type = "ChangeCount"
        value = "1"
        cooldown = "PT1M"
      }
    }
  }
}

##########################
# LOAD BALANCER
##########################

resource "azurerm_public_ip" "blue" {
  count = var.enable_blue ? 1 : 0
  name = "blue"
  location = "${azurerm_resource_group.rg.location}"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  allocation_method = "Static"
  sku = "Standard"

  tags = {
    environment = "Blue Green"
  }
}

resource "azurerm_lb" "blue" {
  count = var.enable_blue ? 1 : 0
  name = "blue"
  location = "${azurerm_resource_group.rg.location}"
  resource_group_name = "${azurerm_resource_group.rg.name}"
  sku = "Standard"

  frontend_ip_configuration {
    name = "PublicIPAddress"
    public_ip_address_id = "${azurerm_public_ip.blue.0.id}"
  }
}

resource "azurerm_lb_rule" "blue" {
  count = var.enable_blue ? 1 : 0
  resource_group_name = "${azurerm_resource_group.rg.name}"
  loadbalancer_id = "${azurerm_lb.blue.0.id}"
  name = "LBRule"
  protocol = "Tcp"
  frontend_port = 80
  backend_port = 9000
  frontend_ip_configuration_name = "PublicIPAddress"
  backend_address_pool_id = "${azurerm_lb_backend_address_pool.blue.0.id}"
  probe_id = "${azurerm_lb_probe.blue.0.id}"
}

resource "azurerm_lb_probe" "blue" {
  count = var.enable_blue ? 1 : 0
  resource_group_name = "${azurerm_resource_group.rg.name}"
  loadbalancer_id = "${azurerm_lb.blue.0.id}"
  name = "http-probe"
  protocol = "Http"
  request_path = "/"
  port = 9000
}

resource "azurerm_lb_backend_address_pool" "blue" {
  count = var.enable_blue ? 1 : 0
  resource_group_name = "${azurerm_resource_group.rg.name}"
  loadbalancer_id = "${azurerm_lb.blue.0.id}"
  name = "BackEndAddressPool"
}
